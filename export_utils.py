...
def export_excel(rows, save_path):
    ...
    ws = wb.active
    ws.title = "Inventaire"
    headers = ["Désignation", "Nature", "Quantité", "Prix", "Seuil mini", "Date ajout", "Observation", "Valeur (Qté*Prix)"]
    ws.append(headers)
...
def export_pdf(rows, save_path, title="Inventaire Magasin"):
    ...
    data = [["Désignation", "Nature", "Quantité", "Prix", "Seuil mini", "Date ajout", "Observation", "Valeur (Qté*Prix)"]]
    for r in rows:
        valeur = (r[2] or 0) * float(r[3] or 0)
